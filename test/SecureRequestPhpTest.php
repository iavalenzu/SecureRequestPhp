...
class SecureRequestPhpTest extends \PHPUnit\Framework\TestCase
{
    public function testGenerateAndVerifyToken()
    {
        ...
        $options = [
            'clientPublicKeyFn' => function($client_id) use ($pubKey) { return $pubKey ; }
        ];

        $out = JWSRequestVerifier::verify($request, $options);
